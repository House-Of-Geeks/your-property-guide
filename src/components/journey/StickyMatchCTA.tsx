...
/**
 * Floating "Get connected" bar that slides up once the user has scrolled
 * into the page. Sits above the mobile bottom nav on small screens, and
 * floats bottom-right on desktop. Dismissible per session.
 *
 * Designed to lift conversion on long suburb pages where the in-page CTA
 * at the bottom doesn't always get reached.
 */
export function StickyMatchCTA({
  ...
  // Build the deep link to homepage match flow with pre-fills
  const qs = new URLSearchParams();
  if (suburb) qs.set("suburb", suburb);
  if (intent) qs.set("intent", intent);
  const href = `/${qs.toString() ? `?${qs.toString()}` : ""}#match`;
