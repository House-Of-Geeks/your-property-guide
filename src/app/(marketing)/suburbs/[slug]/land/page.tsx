...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Land for Sale in ${suburb.name}, ${suburb.state} | ${SITE_NAME}`;
    ...
    <>
      <BreadcrumbJsonLd
        items={[
          ...
          { name: "Land", url: `/suburbs/${slug}/land` },
        ]}
      />
      <PlaceJsonLd
        name={suburb.name}
      ...
      <SuburbSubrouteHeader
        suburb={suburb}
        eyebrow="Listings in"
        title={<><span className="italic text-primary">Land</span> for sale</>}
        subtitle={`${count} ${count === 1 ? "block" : "blocks"} of land on the market in ${suburb.name}, ${suburb.state} ${suburb.postcode}.`}
        breadcrumbLeaf="Land"
        tabs={getSuburbListingTabs(slug, "land")}
      />

      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        <PropertyGrid
          properties={properties}
          emptyMessage={`No land listings in ${suburb.name} right now. Try the other listing types above.`}
        />
      </div>
