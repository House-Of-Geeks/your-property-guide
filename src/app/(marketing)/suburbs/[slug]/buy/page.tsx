...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Zero-stock shells stay reachable (tabs + internal links still work) but
  // are noindexed until inventory lands — same reasoning as the isThinSuburb
  // gate on the suburb profile page.
  const listingCount = await countProperties({ listingType: "buy", suburb: slug });
  return {
...
export default async function SuburbBuyPage({ params }: Props) {
  const { slug } = await params;
  const suburb = await getSuburbBySlug(slug);
  if (!suburb) notFound();
