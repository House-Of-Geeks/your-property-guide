...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Houses for Sale in ${suburb.name}, ${suburb.state} | Your Property Guide`;
  ...
  return {
    ...
    openGraph: { url: `${SITE_URL}/suburbs/${slug}/houses`, title, description, type: "website" },
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
      <BreadcrumbJsonLd
        items={[
          ...
          { name: "Houses for Sale", url: `/suburbs/${slug}/houses` },
        ]}
      />
      <PlaceJsonLd
        name={suburb.name}
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">
          {properties.length} {properties.length === 1 ? "house" : "houses"} for sale in {suburb.name}, {suburb.state} {suburb.postcode}
        </p>
      </div>

      {properties.length === 0 ? (
        <div className="py-12 text-center">
          <p className="text-gray-500 mb-4">No house listings in {suburb.name} right now.</p>
          <Link
        ...
        <PropertyGrid
          ...
          emptyMessage={`No house listings in ${suburb.name} right now.`}
        />
      )}
