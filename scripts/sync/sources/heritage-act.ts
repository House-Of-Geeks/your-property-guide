 ...
 *
 * License: CC BY 4.0 — © Australian Capital Territory Government
 *   "ACT Heritage, CC BY 4.0."
 *
 * Note: Sensitive heritage sites (e.g. Aboriginal places) are de-identified by
 * the upstream publisher — site geometries are snapped to ACT block boundaries
 * rather than exposing precise locations.
 *
 * Source endpoint:
 *   https://services1.arcgis.com/E5n4f1VY84i0xSjy/arcgis/rest/services/ACTGOV_Heritage_Register/FeatureServer/1
 *
 * Field mapping:
 *   NAME       → overlay.label / attrs.name        (e.g. "Old Parliament House")
 *   HRcategory → attrs.heritageType                ("Historic", "Aboriginal", "Natural")
 *
 * CLI flags:
...
const SOURCE_ID = "heritage-act";
const OVERLAY_KIND = "heritage";
const OVERLAY_SOURCE = "act-heritage-register";
const STATE = "ACT";
const ARC_BASE = "https://services1.arcgis.com/E5n4f1VY84i0xSjy/arcgis/rest/services/ACTGOV_Heritage_Register/FeatureServer/1";
const CACHE_DIR = join(process.cwd(), ".cache", "heritage-act");
    ...
    for (const f of resp.features) {
      ...
      for (const poly of polys) {
        ...
        features.push({
          ...
          name:         f.properties.NAME       ? String(f.properties.NAME).trim()       : null,
          heritageType: f.properties.HRcategory ? String(f.properties.HRcategory).trim() : null,
        });
      }
    }
...
async function preflight(): Promise<void> {
  ...
  if (gnaf < 100_000) {
    throw new Error(`Pre-flight failed: only ${gnaf} ACT G-NAF rows with lat/lng (expected ≥ 100k).`);
  }
  log(SOURCE_ID, `pre-flight: ${gnaf.toLocaleString()} ACT G-NAF rows with coordinates ✓`);
        ...
        for (const a of batch) {
          ...
          if (m) {
            ...
            writeBuffer.push({
              addressId: a.id,
              code:      "LISTED",
              label:     m.name ?? "Heritage listed",
              attrs:     JSON.stringify({ heritageType: m.heritageType, name: m.name }),
              effectiveAt: null,
            });
