 ...
 *
 * Field mapping:
 *   ZONE     → overlay.code   (zone name — TAS has no short code, e.g. "Low Density Residential")
 *   ZONE     → overlay.label  (same)
 *   LPS      → attrs.scheme   (e.g. "Glenorchy Local Provisions Schedule")
 *   ZONE_ABB → attrs.zoneAbb  (numeric abbrev like "113.17" for cross-ref)
 *
 * CLI flags:
...
const SOURCE_ID = "zoning-tas";
const OVERLAY_KIND = "zoning";
const OVERLAY_SOURCE = "tas-tps-zoning";
const STATE = "TAS";
const ARC_BASE = "https://data.stategrowth.tas.gov.au/ags/rest/services/PROJECTS/PLANNINGSCHEMEZONES_TPC/FeatureServer/0";
const CACHE_DIR = join(process.cwd(), ".cache", "zoning-tas");
    ...
    for (const f of resp.features) {
      ...
      for (const poly of polys) {
        ...
        features.push({
          ...
          zoneCode:  String(f.properties.ZONE ?? "").trim(),
          zoneLabel: f.properties.ZONE     ? String(f.properties.ZONE).trim()     : null,
          scheme:    f.properties.LPS      ? String(f.properties.LPS).trim()      : null,
          zoneAbb:   f.properties.ZONE_ABB ? String(f.properties.ZONE_ABB).trim() : null,
        });
      }
    }
        ...
        for (const a of batch) {
          ...
          if (m) {
            ...
            writeBuffer.push({
              ...
              code:      m.zoneCode,
              label:     m.zoneLabel,
              attrs:     JSON.stringify({ scheme: m.scheme, zoneAbb: m.zoneAbb }),
              effectiveAt: null,
            });
