/**
 * VIC Planning Scheme Flood Overlay Ingest (Vicmap Planning — Planning Scheme Overlay Polygon)
 *
 * Filters the planning overlay layer by zone_code IN ('LSIO','FO','SBO') to capture
 * flood-relevant overlays:
 *   LSIO — Land Subject to Inundation Overlay
 *   FO   — Floodway Overlay
 *   SBO  — Special Building Overlay (stormwater / urban flooding)
 *
 * License: CC BY 4.0 — © State of Victoria (Department of Transport and Planning)
 *   "Vicmap Planning — Planning Scheme Overlay, licensed under CC BY 4.0."
 *
 * Source endpoint:
 *   https://services-ap1.arcgis.com/P744lA0wf4LlBZ84/ArcGIS/rest/services/Vicmap_Planning/FeatureServer/2
 *
 * Field mapping:
 *   zone_code        → overlay.code         (e.g. "LSIO", "FO", "SBO")
 ...
 *   --no-cache           — re-fetch overlay polygons from network
 */
...
const SOURCE_ID = "flood-vic";
const OVERLAY_KIND = "flood";
const OVERLAY_SOURCE = "vic-vicmap-flood";
const STATE = "VIC";
const ARC_BASE = "https://services-ap1.arcgis.com/P744lA0wf4LlBZ84/ArcGIS/rest/services/Vicmap_Planning/FeatureServer/2";
const CACHE_DIR = join(process.cwd(), ".cache", "flood-vic");
