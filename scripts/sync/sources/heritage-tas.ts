/**
 * TAS Heritage Register Ingest (Tasmanian Heritage Register)
 *
 ...
 *
 * Field mapping:
 *   THR_NAME   → overlay.label  (heritage place name)
 *   THR_ID     → attrs.placeId  (Tasmanian Heritage Register unique ID)
 *   REG_STATUS → attrs.heritageType  (registration status, e.g. "Permanently Registered")
 *
 * CLI flags:
...
const SOURCE_ID = "heritage-tas";
const OVERLAY_KIND = "heritage";
const OVERLAY_SOURCE = "tas-heritage-register";
const STATE = "TAS";
const ARC_BASE = "https://services.thelist.tas.gov.au/arcgis/rest/services/HT/HT_Public/MapServer/0";
const CACHE_DIR = join(process.cwd(), ".cache", "heritage-tas");
    ...
    for (const f of resp.features) {
      ...
      for (const poly of polys) {
        ...
        features.push({
          ...
          name:         String(f.properties.THR_NAME ?? "").trim(),
          placeId:      f.properties.THR_ID     ? String(f.properties.THR_ID).trim()     : null,
          heritageType: f.properties.REG_STATUS ? String(f.properties.REG_STATUS).trim() : null,
        });
      }
    }
