/**
 * QLD FloodCheck 1% AEP Riverine Flood Layer
 *
 * Same architecture as zoning-vic.ts; only endpoint and field mapping differ.
 *
 * License: CC BY 4.0 — © State of Queensland (Department of Resources / FloodCheck)
 *   "Queensland FloodCheck Basin 1% AEP, licensed under CC BY 4.0."
 *
 * Source endpoint:
 *   https://spatial-gis.information.qld.gov.au/arcgis/rest/services/FloodCheck/BasinOnePercentAEP/MapServer/1
 *
 * Caveat: coverage is study-by-study (basin-level), not statewide. Each polygon
 * represents a basin study extent where modelled 1% AEP riverine flood
 * mapping has been completed. Address-level "no match" therefore means
 * "no FloodCheck study covers this point" — NOT "no flood risk".
 *
 * Field mapping:
 *   TOWN             → overlay.code         (basin name, used as study identifier)
 *   STATUS           → attrs.status         (e.g. "COMPLETE")
 *   RIVER_GAUGE      → attrs.riverGauge
 *   RIVER_GAUGE_TYPE → attrs.riverGaugeType (BOM | DERM)
 *   Bundle           → attrs.bundle
 *
 * CLI flags:
 ...
 *   --postcode=4000      — process only this postcode (debug)
 *   --no-cache           — re-fetch flood polygons from network
 */
...
const SOURCE_ID = "flood-qld";
const OVERLAY_KIND = "flood";
const OVERLAY_SOURCE = "qld-floodcheck";
const STATE = "QLD";
const ARC_BASE = "https://spatial-gis.information.qld.gov.au/arcgis/rest/services/FloodCheck/BasinOnePercentAEP/MapServer/1";
const CACHE_DIR = join(process.cwd(), ".cache", "flood-qld");
...
interface OverlayFeature {
  ...
  town: string;                             // basin name (used as code)
  status: string | null;
  riverGauge: string | null;
  riverGaugeType: string | null;
  bundle: number | null;
}
...
async function downloadPolygons(useCache: boolean): Promise<OverlayFeature[]> {
  ...
  log(SOURCE_ID, `fetching flood polygons from ${ARC_BASE}`);
  ...
  while (exceeded) {
    ...
    for (const f of resp.features) {
      // Null-geometry guard — QLD MapServer occasionally returns features with
      // null geometry when a basin's source raster lacks a vector footprint.
      ...
      for (const poly of polys) {
        ...
        const bundleRaw = f.properties.Bundle;
        features.push({
          bbox: [minLng, minLat, maxLng, maxLat],
          ring,
          rings: poly,
          area: (maxLng - minLng) * (maxLat - minLat),
          town:           String(f.properties.TOWN ?? "").trim(),
          status:         f.properties.STATUS           ? String(f.properties.STATUS).trim()           : null,
          riverGauge:     f.properties.RIVER_GAUGE      ? String(f.properties.RIVER_GAUGE).trim()      : null,
          riverGaugeType: f.properties.RIVER_GAUGE_TYPE ? String(f.properties.RIVER_GAUGE_TYPE).trim() : null,
          bundle:         typeof bundleRaw === "number" ? bundleRaw : (bundleRaw != null ? Number(bundleRaw) : null),
        });
      }
    }
  ...
  log(SOURCE_ID, `downloaded ${features.length} flood polygons total`);
...
function matchPoint(tree: RBush<RbushEntry>, lng: number, lat: number): MatchResult | null {
  ...
  // Prefer the smallest-area polygon when multiple match (most specific study).
  ...
  for (const c of sorted) {
    ...
    if (booleanPointInPolygon(pt, polygon)) {
      return {
        ...
        riverGauge:     c.feature.riverGauge,
        riverGaugeType: c.feature.riverGaugeType,
        bundle:         c.feature.bundle,
      };
    }
  }
  return null;
...
async function writeBatch(
  ...
  try {
    ...
    // Upsert: study coverage changes flow through, runId always reflects latest pass
...
async function preflight(): Promise<void> {
  ...
  if (gnaf < 100_000) {
    throw new Error(`Pre-flight failed: only ${gnaf} QLD G-NAF rows with lat/lng (expected ≥ 100k).`);
  }
  log(SOURCE_ID, `pre-flight: ${gnaf.toLocaleString()} QLD G-NAF rows with coordinates ✓`);
  ...
  log(SOURCE_ID, `pre-flight: ArcGIS reports ${json.count?.toLocaleString() ?? "?"} flood polygons available ✓`);
}
...
export async function run(): Promise<void> {
  ...
  try {
    ...
    if (features.length === 0) throw new Error("No flood polygons downloaded — aborting.");
        ...
        for (const a of batch) {
          ...
          if (m) {
            ...
            writeBuffer.push({
              addressId: a.id,
              code:      "1% AEP",
              label:     "Modelled 1% AEP riverine flood",
              attrs:     JSON.stringify({
                town:           m.town,
                status:         m.status,
                riverGauge:     m.riverGauge,
                riverGaugeType: m.riverGaugeType,
                bundle:         m.bundle,
              }),
              effectiveAt: null,
            });
    ...
    log(SOURCE_ID, `note: unmatched does NOT mean "no flood risk" — QLD FloodCheck coverage is study-by-study, not statewide.`);
