/**
 * QLD Heritage Register Ingest (Queensland Heritage Register)
 *
 * Same architecture as heritage-tas.ts; only endpoint and field mapping differ.
 *
 * License: CC BY 4.0 — Queensland Government open data
 * Attribution: "© State of Queensland (Department of Environment, Tourism, Science and Innovation)"
 *
 * Source endpoint:
 *   https://spatial-gis.information.qld.gov.au/arcgis/rest/services/Boundaries/AdminBoundariesFramework/MapServer/78
 *
 * Field mapping:
 *   placename → overlay.label  (heritage place name)
 *   place_id  → attrs.placeId  (Queensland Heritage Register unique ID, integer)
 *   status    → attrs.heritageType  (e.g. "State heritage place")
 *
 * CLI flags:
 ...
 *   --no-cache           — re-fetch heritage polygons from network
 */
...
const SOURCE_ID = "heritage-qld";
const OVERLAY_KIND = "heritage";
const OVERLAY_SOURCE = "qld-heritage-register";
const STATE = "QLD";
const ARC_BASE = "https://spatial-gis.information.qld.gov.au/arcgis/rest/services/Boundaries/AdminBoundariesFramework/MapServer/78";
const CACHE_DIR = join(process.cwd(), ".cache", "heritage-qld");
const CACHE_FILE = join(CACHE_DIR, "polygons.jsonl");

const ARC_PAGE_SIZE = 1000;          // ArcGIS maxRecordCount (server caps at 2000)
...
async function downloadPolygons(useCache: boolean): Promise<OverlayFeature[]> {
  ...
  log(SOURCE_ID, `fetching heritage polygons from ${ARC_BASE}`);
  ...
  while (exceeded) {
    ...
    for (const f of resp.features) {
      // Null-geometry guard: some heritage entries may be registered without spatial data.
      ...
      for (const poly of polys) {
        ...
        features.push({
          ...
          name:         String(f.properties.placename ?? "").trim(),
          placeId:      f.properties.place_id != null ? String(f.properties.place_id).trim() : null,
          heritageType: f.properties.status     ? String(f.properties.status).trim()     : null,
        });
      }
    }
  ...
  log(SOURCE_ID, `downloaded ${features.length} heritage polygons total`);
...
function matchPoint(tree: RBush<RbushEntry>, lng: number, lat: number): MatchResult | null {
  ...
  for (const c of sorted) {
    ...
    if (booleanPointInPolygon(pt, polygon)) {
      return {
        name:         c.feature.name,
        placeId:      c.feature.placeId,
        heritageType: c.feature.heritageType,
      };
    }
  }
  return null;
...
async function preflight(): Promise<void> {
  ...
  log(SOURCE_ID, `pre-flight: ArcGIS reports ${json.count?.toLocaleString() ?? "?"} heritage polygons available ✓`);
}
...
export async function run(): Promise<void> {
  ...
  try {
    ...
    if (features.length === 0) throw new Error("No heritage polygons downloaded — aborting.");
        ...
        for (const a of batch) {
          ...
          if (m) {
            ...
            writeBuffer.push({
              ...
              code:      "LISTED",
              label:     m.name,
              attrs:     JSON.stringify({ name: m.name, placeId: m.placeId, heritageType: m.heritageType }),
              effectiveAt: null,
            });
