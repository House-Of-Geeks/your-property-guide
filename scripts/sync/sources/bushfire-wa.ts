/**
 * WA Bush Fire Prone Area Planning (OBRM-023)
 *
 * Parcel-level bushfire prone area ingest for Western Australia. Same
 * architecture as zoning-vic.ts; only endpoint and field mapping differ.
 *
 * License: CC BY 4.0 — © Western Australian Land Information Authority
 *   (Landgate). OBRM-023 "Map of Bush Fire Prone Areas" is the only WA
 *   bushfire dataset released under a strictly CC BY 4.0 licence.
 *
 * Note: WA's designated Bush Fire Prone Area polygons include a 100m
 * vegetation buffer around classified bushfire-prone vegetation, in line
 * with the FES Commissioner's designation methodology. A property
 * matching here is inside the statutory designation (most recent
 * designation: 13 Dec 2025 by the FES Commissioner).
 *
 * Source endpoint:
 *   https://services.slip.wa.gov.au/arcgis/rest/services/Landgate_Public_Maps/Map_of_Bush_Fire_Prone_Areas_3/MapServer/8
 *
 * Field mapping:
 *   type             → overlay.code         (e.g. "BPA")
 *   designation      → attrs.designation    (long-form designation text)
 *   planningarea     → attrs.planningArea   (e.g. "Bushfire Prone Area 1"/"...2")
 *   designationdate  → overlay.effectiveAt  (epoch ms)
 *   lga              → attrs.lga
 *
 * CLI flags:
 ...
 *   --postcode=6000      — process only this postcode (debug)
...
const SOURCE_ID = "bushfire-wa";
const OVERLAY_KIND = "bushfire";
const OVERLAY_SOURCE = "wa-obrm-bfpa";
const STATE = "WA";
const ARC_BASE = "https://services.slip.wa.gov.au/arcgis/rest/services/Landgate_Public_Maps/Map_of_Bush_Fire_Prone_Areas_3/MapServer/8";
const CACHE_DIR = join(process.cwd(), ".cache", "bushfire-wa");
...
interface OverlayFeature {
  ...
  typeCode: string;                         // e.g. "BPA"
  designation: string | null;               // long-form designation text
  planningArea: string | null;              // e.g. "Bushfire Prone Area 1"
  designationDate: Date | null;             // statutory designation date
  lga: string | null;
}
...
async function downloadPolygons(useCache: boolean): Promise<OverlayFeature[]> {
  ...
  if (useCache && existsSync(CACHE_FILE)) {
    ...
    for await (const line of rl) {
      if (line) {
        const parsed = JSON.parse(line) as Omit<OverlayFeature, "designationDate"> & { designationDate: string | null };
        cached.push({
          ...parsed,
          designationDate: parsed.designationDate ? new Date(parsed.designationDate) : null,
        });
      }
    }
  ...
  while (exceeded) {
    ...
    for (const f of resp.features) {
      // Null-geometry guard: ArcGIS occasionally returns features whose
      // geometry serialised to null (clipping artefacts, invalid rings).
      ...
      for (const poly of polys) {
        ...
        const rawDate = f.properties.designationdate;
        const designationDate = rawDate != null && rawDate !== ""
          ? new Date(typeof rawDate === "number" ? rawDate : Number(rawDate))
          : null;
        ...
        features.push({
          ...
          typeCode:        String(f.properties.type ?? "BPA").trim() || "BPA",
          designation:     f.properties.designation  ? String(f.properties.designation).trim()  : null,
          planningArea:    f.properties.planningarea ? String(f.properties.planningarea).trim() : null,
          designationDate: designationDate && !isNaN(designationDate.getTime()) ? designationDate : null,
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    for (const f of features) {
      const serialisable = {
        ...f,
        designationDate: f.designationDate ? f.designationDate.toISOString() : null,
      };
      ws.write(JSON.stringify(serialisable) + "\n");
    }
    ws.end();
  });
...
function matchPoint(tree: RBush<RbushEntry>, lng: number, lat: number): MatchResult | null {
  ...
  // Prefer the smallest-area polygon when multiple match (most specific designation).
  ...
  for (const c of sorted) {
    ...
    if (booleanPointInPolygon(pt, polygon)) {
      return {
        typeCode:        c.feature.typeCode,
        designation:     c.feature.designation,
        planningArea:    c.feature.planningArea,
        designationDate: c.feature.designationDate,
        lga:             c.feature.lga,
      };
    }
  }
...
async function writeBatch(
  ...
  try {
    ...
    // Upsert: designation changes flow through, runId always reflects latest pass
...
async function preflight(): Promise<void> {
  ...
  if (gnaf < 500_000) {
    throw new Error(`Pre-flight failed: only ${gnaf} WA G-NAF rows with lat/lng (expected ≥ 500k; WA has ~1.57M).`);
  }
  log(SOURCE_ID, `pre-flight: ${gnaf.toLocaleString()} WA G-NAF rows with coordinates ✓`);
  ...
  log(SOURCE_ID, `pre-flight: ArcGIS reports ${json.count?.toLocaleString() ?? "?"} bushfire polygons available ✓`);
}
        ...
        for (const a of batch) {
          ...
          if (m) {
            ...
            writeBuffer.push({
              addressId: a.id,
              code:      m.typeCode || "BPA",
              label:     "Bush Fire Prone Area (designated)",
              attrs:     JSON.stringify({
                designation:  m.designation,
                planningArea: m.planningArea,
                lga:          m.lga,
              }),
              effectiveAt: m.designationDate,
            });
