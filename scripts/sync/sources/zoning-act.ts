/**
 * ACT Territory Plan Land Use Zone Ingest
 *
 ...
 *   "ACT Government Geospatial Data Catalogue (ACTmapi), CC BY 4.0."
 *
 * Source endpoint:
 *   https://services1.arcgis.com/E5n4f1VY84i0xSjy/arcgis/rest/services/ACTGOV_TP_LAND_USE_ZONE/FeatureServer/1
 *
 * Field mapping:
 *   LAND_USE_ZONE_CODE_ID → overlay.code   (e.g. "RZ1", "CZ1")
 *   LAND_USE_POLICY_DESC  → overlay.label  ("SUBURBAN", "MIXED USE", etc.)
...
const SOURCE_ID = "zoning-act";
const OVERLAY_KIND = "zoning";
const OVERLAY_SOURCE = "act-tp-zoning";
const STATE = "ACT";
const ARC_BASE = "https://services1.arcgis.com/E5n4f1VY84i0xSjy/arcgis/rest/services/ACTGOV_TP_LAND_USE_ZONE/FeatureServer/1";
const CACHE_DIR = join(process.cwd(), ".cache", "zoning-act");
    ...
    for (const f of resp.features) {
      ...
      for (const poly of polys) {
        ...
        features.push({
          ...
          zoneCode:  String(f.properties.LAND_USE_ZONE_CODE_ID ?? "").trim(),
          zoneLabel: f.properties.LAND_USE_POLICY_DESC ? String(f.properties.LAND_USE_POLICY_DESC).trim() : null,
          division:  f.properties.DIVISION_NAME       ? String(f.properties.DIVISION_NAME).trim()       : null,
        });
      }
    }
        ...
        for (const a of batch) {
          ...
          if (m) {
            ...
            writeBuffer.push({
              ...
              code:      m.zoneCode,
              label:     m.zoneLabel,
              attrs:     JSON.stringify({ division: m.division }),
              effectiveAt: null,
            });
