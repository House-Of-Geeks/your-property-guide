/**
 * NSW Land Zoning Ingest (Environmental Planning Instrument — Land Zoning)
 *
 * Pipeline:
 *   1. Paginate-download all NSW zoning polygons from the Planning Portal's
 *      ArcGIS REST FeatureServer (~50k polygons, GeoJSON, ~1 min).
 ...
 *     "Land Zoning data © State of New South Wales (Department of Planning,
...
const SOURCE_ID = "zoning-nsw";
const OVERLAY_KIND = "zoning";
const OVERLAY_SOURCE = "nsw-epi-zoning";
const STATE = "NSW";
const ARC_BASE = "https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/2";
const CACHE_DIR = join(process.cwd(), ".cache", "zoning-nsw");
    ...
    for (const f of resp.features) {
      ...
      for (const poly of polys) {
        ...
        features.push({
          ...
          zoneCode:  String(f.properties.SYM_CODE  ?? "").trim(),
          zoneLabel: f.properties.LAY_CLASS ? String(f.properties.LAY_CLASS).trim() : null,
        ...
        for (const a of batch) {
          ...
          if (m) {
            ...
            writeBuffer.push({
              ...
              code:      m.zoneCode,
              label:     m.zoneLabel,
              attrs:     JSON.stringify({ epiName: m.epiName }),
              effectiveAt: null,
            });
