/**
 * NSW Parcel-Level Flood Ingest (Flood Planning Map — Hazard layer)
 *
 * Pipeline:
 *   1. Paginate-download all NSW flood-affected polygons from the Planning
 *      Portal's ArcGIS REST MapServer (~622 polygons, GeoJSON).
 ...
 *      so the most specific flood designation prevails).
 *   5. COPY into PropertyOverlay (kind='flood', source='nsw-flood-planning')
 *      via temp table + INSERT ON CONFLICT DO UPDATE.
 *
 * Field meaning (NSW Planning Portal Hazard layer 230 — Flood Planning Map):
 *   - LAY_CLASS: the flood category that designates the parcel as flood-prone.
 *     Observed values include:
 *       "Flood Planning Area"             → most common — within a council's
 *                                           defined Flood Planning Area
 *       "Flood Prone and Major Creeks Land"
 *       "1 in 100 AEP Flood Extent"       → 1% AEP / 1-in-100-year extent
 *       "Probable Maximum Flood Line"
 *       "Level of Probable Maximum Flood"
 *       "Land Identified in Section 3.27" → SEPP-defined flood land
 *       "Area 1", "Transitional Land",
 *       "Cultural Heritage Landscape Area"
 *   - EPI_NAME: the LEP/SEPP defining the flood requirement.
 *   - LGA_NAME, EPI_TYPE: contextual attributes.
 *
 * Idempotency:
 *   - (addressId, kind, source) is unique → ON CONFLICT DO UPDATE refreshes the
 *     row in place (flood category changes flow through cleanly).
 ...
 *     "Flood Planning Map data © State of New South Wales (Department of
 *      Planning, Housing and Infrastructure), licensed under CC BY 4.0."
 *
 * Source endpoint:
...
const SOURCE_ID = "flood-nsw";
const OVERLAY_KIND = "flood";
const OVERLAY_SOURCE = "nsw-flood-planning";
const STATE = "NSW";
const ARC_BASE = "https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/ePlanning/Planning_Portal_Hazard/MapServer/230";
const CACHE_DIR = join(process.cwd(), ".cache", "flood-nsw");
...
interface OverlayFeature {
  ...
  layClass: string;                         // flood category (e.g. "Flood Planning Area")
  epiName:  string | null;
  epiType:  string | null;
  lgaName:  string | null;
}
...
async function downloadPolygons(useCache: boolean): Promise<OverlayFeature[]> {
    ...
    for (const f of resp.features) {
      ...
      for (const poly of polys) {
        ...
        features.push({
          ...
          layClass: String(f.properties.LAY_CLASS ?? "").trim(),
          epiName:  f.properties.EPI_NAME ? String(f.properties.EPI_NAME).trim() : null,
          epiType:  f.properties.EPI_TYPE ? String(f.properties.EPI_TYPE).trim() : null,
          lgaName:  f.properties.LGA_NAME ? String(f.properties.LGA_NAME).trim() : null,
        });
      }
    }
  ...
  // Stream-write JSONL (V8's string length cap blows up on JSON.stringify of large arrays)
...
function matchPoint(tree: RBush<RbushEntry>, lng: number, lat: number): MatchResult | null {
  ...
  // Prefer the smallest-area polygon when multiple match (most specific category).
  ...
  for (const c of sorted) {
    ...
    if (booleanPointInPolygon(pt, polygon)) {
      return {
        layClass: c.feature.layClass,
        epiName:  c.feature.epiName,
        epiType:  c.feature.epiType,
        lgaName:  c.feature.lgaName,
      };
    }
  }
  return null;
...
// Map the raw LAY_CLASS string to a short stable code suitable for storage.
// Anything not in the map falls back to a slugified version of the class.
...
async function writeBatch(
  ...
  try {
    ...
    // Upsert: flood category changes flow through, runId always reflects latest pass
      ...
      while (true) {
        ...
        for (const a of batch) {
          ...
          if (m) {
            ...
            writeBuffer.push({
              addressId: a.id,
              code:      classToCode(m.layClass),
              label:     m.layClass || null,
