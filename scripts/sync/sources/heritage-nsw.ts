/**
 * NSW Heritage Register / Heritage Items Ingest
 *
 * Pipeline:
 *   1. Paginate-download all NSW heritage polygons from the Planning Portal's
 *      ArcGIS REST MapServer (~40k polygons, GeoJSON, ~1 min).
 ...
 *   5. COPY into PropertyOverlay (kind='heritage', source='nsw-heritage-register')
 *      via temp table + INSERT ON CONFLICT DO UPDATE.
 *
 * Idempotency:
 *   - (addressId, kind, source) is unique → ON CONFLICT DO UPDATE refreshes the
 *     row in place (heritage status changes flow through cleanly).
 ...
 *     "Heritage data © State of New South Wales (Department of Planning,
...
const SOURCE_ID = "heritage-nsw";
const OVERLAY_KIND = "heritage";
const OVERLAY_SOURCE = "nsw-heritage-register";
const STATE = "NSW";
const ARC_BASE = "https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/Principal_Planning_Layers/MapServer/8";
const CACHE_DIR = join(process.cwd(), ".cache", "heritage-nsw");
    ...
    for (const f of resp.features) {
      ...
      for (const poly of polys) {
        ...
        features.push({
          ...
          name:          String(f.properties.H_NAME ?? "").trim(),
          heritageLevel: f.properties.SIG ? String(f.properties.SIG).trim() : null,
        });
      }
    }
...
function matchPoint(tree: RBush<RbushEntry>, lng: number, lat: number): MatchResult | null {
  ...
  for (const c of sorted) {
    ...
    if (booleanPointInPolygon(pt, polygon)) {
      return {
        name:          c.feature.name,
        heritageLevel: c.feature.heritageLevel,
      };
    }
  }
  return null;
      ...
      while (true) {
        ...
        for (const a of batch) {
          ...
          if (m) {
            ...
            writeBuffer.push({
              addressId: a.id,
              code:      m.heritageLevel ?? "LISTED",
              label:     m.name,
              attrs:     JSON.stringify({ name: m.name, heritageLevel: m.heritageLevel }),
              effectiveAt: null,
            });
